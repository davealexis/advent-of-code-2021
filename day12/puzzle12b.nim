...
proc findPaths(graph: seq[Edge]): int =
    var
        queue: Deque[QEntry]
        startNode: QEntry = ("start", toHashSet(["start"]), 0)
        pathCount = 0
        iterations = 0
    ...
    while queue.len > 0:
        ...
        for edge in graph.filter(n => n.s == node.name):
            ...
            elif edge.e in node.visited and edge.e != "end" and edge.e != "start" and node.visitCount == 0:
                var newNode: QEntry
                newNode.name = edge.e
                newNode.visited = node.visited
        ...
        if iterations mod 10000 == 0:
            stdout.write &"\r{iterations} Paths: {pathCount}"
